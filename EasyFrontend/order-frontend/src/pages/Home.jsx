...
function OrderPage() {
  ...
  return (
    <div className="p-4 relative font-montserrat">
      ...
      <h2 className="text-1xl mb-6">Masa: {deskName}</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {products[type].map((p) => (
              <div key={p.id} className="border p-4 rounded-lg shadow hover:shadow-lg transition duration-300">
                ...
                <p className="text-l mb-2">{p.price} ₺</p>
                <button
                  className="mt-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded text-l"
                  onClick={() => addToCart(p)}
                >
                  Sepete Ekle
                </button>
