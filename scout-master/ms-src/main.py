...
gpio_motor1 = 12 # front left, clockwise
gpio_motor2 = 13 # front right, counter clockwise
gpio_motor3 = 14 # rear left, counter clockwise
...
def run() -> None:
    ...
    led = machine.Pin(2, machine.Pin.OUT) # the onboard LED of the Raspberry Pi Pico
    for x in range(8):
        led.on()
        time.sleep(0.1)
        led.off()
    ...
    print("Overclocked to 160,000,000")
    ...
    for x in range(0, 60):
        rc_data = rc.read()
        if int(packet[3]) == 0: # flight mode on
    ...
    try:
        while True:
            ...
            rc_data = rc.read()
            packetSize = UDP.readinto(packet, 4)
            if packetSize:
            # normalize all RC input values
                input_throttle:float = normalize(int(packet[0]), 1000.0, 2000.0, 0.0, 1.0) # between 0.0 and 1.0
                input_pitch:float = (normalize(int(packet[1]), 1000.0, 2000.0, -1.0, 1.0)) * -1 # between -1.0 and 1.0. We multiply by -1 because... If the pitch is "full forward" (i.e. 75), that means we want a NEGATIVE pitch (when a plane pitches it's nose down, that is negative, not positive. And when a place pitches it's nose up, pulling back on the stick, it's positive, not negative.) Thus, we need to flip it.
                input_roll:float = normalize(int(packet[2])*24, 1000.0, 2000.0, -1.0, 1.0) # between -1.0 and 1.0
                input_yaw:float = normalize(0, 1000.0, 2000.0, -1.0, 1.0) # between -1.0 and 1.0
            ...
            if int(packet[3]) == 0: # standby mode - switch in "up" or OFF position
            ...
            elif int(packet[3]) == 1: # flight mode (idle props at least) - swith in "down" or ON position
                ...
                t3:float = adj_throttle - pid_pitch + pid_roll +ll pid_yaw
                ...
                print("Channel 5 input '" + str(int(packet[3])) + "' not valid. Is the transmitter turned on and connected?")
